...
class Dataset(object):
    ...
    def fromIterable(cls, iterable, limit=None, names=None):
        ...
        Each row must have the same number of fields with the same types (``numbers.Real`` or ``str``).
...
class TreeNode(object):
    ...
    def splitUntil(self, condition, depth=1):
        ...
        If the predictand is categorical (``str``), the node has attributes: ``datasetSize``, ``predictandDistribution``, ``entropy``, and ``gain``.
    ...
    def splitOnce(self):
        ...
        If the predictand is numerical (``numbers.Real``), the split minimizes entropy; if categorical (``str``), it minimizes n-times-variance.
        """
